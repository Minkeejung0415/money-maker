"""
MLBSGPBuilder — generates positive-EV Same-Game Parlay combinations for MLB.

Mirrors SGPBuilder exactly with MLB-specific:
  - Static correlation table for common MLB market pairs
...
Static correlation table:
  same_batter hits + same_batter rbis     → r=0.70
  same_team hits   + team_win             → r=0.55
  pitcher_strikeouts + opponent_hits      → r=-0.40

(Empirical correlation from pybaseball added in later milestone.)
"""
...
# Static MLB correlation table
_STATIC_CORR: dict[tuple[str, str], float] = {
    # Same batter: hits and RBIs positively correlated
    ("batter_hits", "batter_rbis"): 0.70,
    # Home run and RBI positively correlated
    ("batter_home_runs", "batter_rbis"): 0.60,
    # Hits and home runs
    ("batter_hits", "batter_home_runs"): 0.35,
    # Same team hits → team win positively correlated
    ("batter_hits", "team_win"): 0.55,
    # Pitcher strikeouts negatively correlated with opponent hits
    ("batter_hits", "pitcher_strikeouts"): -0.40,
    # Pitcher outs negatively correlated with hits
    ("batter_hits", "pitcher_outs"): -0.30,
}
                ...
                for pl in prop_legs_only:
                    if pl.home_team == team or pl.away_team == team:
                        corr_note_parts.append(
                            "ML+batter same team: positively correlated"
                        )
                        break
    ...
    def _build_corr_note(self, legs: list[PropLeg]) -> str:
        ...
        for i in range(len(legs)):
            for j in range(i + 1, len(legs)):
                ...
                if r != 0.0:
                    parts.append(f"{la.player} vs {lb.player}: r={r:.2f}")
        return "; ".join(parts)
